...
外部 errno location 绑定与基于它读取当前 errno 的 helper，都应放进
`linux/darwin/android/freebsd/unix` 等 host ffi owner 单元，并优先使用宿主前缀命名，例如
`linux_errno_location` / `linux_errno_value`。shared `posix.ffi` 只保留参数化 skeleton，例如
`platform_posix_errno_value_from_location`；`platform.thread`、`platform.sync` 等 consumer 不直接在
实现层解引用 errno storage。
...
monotonic / realtime / resolution 结果。更理想的边界是由各宿主 ffi owner 直接提供宿主前缀的
高层 clock helper，例如 `linux_clock_monotonic_ns_u64`、`linux_clock_realtime_ns_u64`、
`linux_clock_monotonic_resolution_ns_u64`；`platform.time` 只做薄 delegation。历史遗留的
非 Linux POSIX host `platform_clock_*` helper 已收敛为 `android_clock_*`、`darwin_clock_*`、
`freebsd_clock_*` 和 `unix_clock_*`。Windows 时钟 helper 同样归 `windows_clock_*` owner。
后续新 host-owned helper 必须继续使用宿主前缀，不能退回统一 public contract 风格的
`platform_*` 命名。
...
因此，`linux/android/darwin/freebsd/unix.ffi` 这类 host ffi owner 应暴露宿主前缀的 clock helper，
例如 `linux_clock_monotonic_now`、`linux_clock_realtime_now`、`linux_clock_monotonic_getres` 以及
更高层的 `linux_clock_monotonic_ns_u64`、`linux_clock_realtime_ns_u64`、
`linux_clock_monotonic_resolution_ns_u64`。这些 helper 的实现可以是对 shared `posix.ffi` clock
helper 的薄委托。`platform.time` consumer 不再直接写 raw `clock_gettime` / `clock_getres`、
宿主 clock id，也尽量不再自己保留宿主专用 timespec 拼装流程。
...
shared `platform_posix_pthread_mutex_init_kind`。各 host ffi owner 继续只保留宿主前缀 errno
symbol binding、`PLATFORM_POSIX_E*`、`PLATFORM_PTHREAD_MUTEX_*_KIND` 这类宿主 truth，不再各自
复制同一份 load/public-result/public-kind projection skeleton。
...
`PLATFORM_PTHREAD_TIMEOUT_CLOCK_ID` 与宿主前缀 errno location 这类宿主 truth，再把这些 truth 作为
参数交给 shared helper。

因此，host ffi owner 应优先暴露宿主前缀 helper，例如 `linux_pthread_timeout_deadline_after_ns`、
`linux_pthread_timeout_remaining_ns_u64`、`linux_pthread_mutex_init_platform_kind`、
`linux_pthread_mutex_*`、`linux_pthread_rwlock_*`、`linux_pthread_condvar_*`、
`linux_pthread_sync_result`。`platform.sync` 只继续保留 NextPas 的 public result adapter、wait-bucket 策略
...
`platform.thread` 不直接消费低层 pthread create/join/detach raw helper，也不在 consumer 中保存
...
host `.ffi` 的宿主前缀 `*_pthread_state_create/join/detach` helper；shared `posix.ffi` 只提供对
...
宿主前缀 yield/sleep/TLS helper，例如 `linux_pthread_yield`、`linux_pthread_sleep_ns` 与
`linux_pthread_tls_*`。
