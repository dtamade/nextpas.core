...
可以继续拥有跨平台可复用的安全换算函数，但不应继续在 consumer 里散落 raw `mach_*` / QPC /
FILETIME 调用、宿主计数器初始化细节或 `FILETIME` realtime 投影逻辑。

POSIX 的 `clock_gettime` / `clock_getres` 也遵循同一条 owner boundary。shared
`nextpas.core.platform.posix.ffi` 只保留 `timespec` 与 raw ABI declaration；具体宿主使用哪组
clock id、如何把失败投影成 nextPas `Int32`、以及 monotonic / realtime / resolution 的 raw
clock 调用细节，应归 `linux/android/darwin/freebsd/unix.ffi` 这类 host ffi owner 单元提供
`platform_clock_monotonic_now`、`platform_clock_realtime_now`、
`platform_clock_monotonic_getres` helper。`platform.time` consumer 只保留跨平台 timespec 到
纳秒的换算与 public clock contract，不再直接写 raw `clock_gettime` / `clock_getres` 或宿主
clock id。
